...
var util = __webpack_require__(1),
	...
	InStream = __webpack_require__(72),
	OutStream = __webpack_require__(73),
...
var util = __webpack_require__(1),
	net = __webpack_require__(27),
	tls = __webpack_require__(28),
	events = __webpack_require__(24),
	Connection
...
var Server = __webpack_require__(22),
	Connection = __webpack_require__(21),
	net = __webpack_require__(27),
	tls = __webpack_require__(28),
...
var server = ws.createServer(options, function (socket) {
    ...
    socket.on('text', function (str) {
        ...
        switch (data.event) {
            ...
            case 'StartShare':
                var conS = allSockets[data.shareName];
                sendTo(conS, {
                    event: 'share',
                    name: data.name,
                ...
                break;
            case 'closeShare':
                var con = allSockets[data.shareName];
                if (writerShare.length) {
                    writerShare = writerShare.filter(function (item) {
                        return item.name != data.shareName;
                    });
                }
                sendTo(con, {
                    event: 'closedShare',
            ...
            case 'join':
                ...
                // Save users info 
                if (allUsers.findIndex(function (u) {
                    return u.name === _user;
                }) < 0) {
                    allUsers.push({ name: _user, status: 1, subName: data.subName }); // '0'-离线 '1'-在线 '2'-通话中
                }
                broadcast(server, {
                    event: 'join',
                    allUsers: allUsers,
                ...
                if (writerShare.length) {
                    writerShare.forEach(function (data) {
                        if (data.name == _user) {
                            var _conn = allSockets[data.name];
            ...
            case 'createRoom':
                if (data.mode !== 'unilaterally') {
                    ...
                    if (!!data.partner) {
                        data.partner.forEach(function (user) {
            ...
            case 'updateRoom':
                if (!!room[data.roomId]) {
                    ...
                    data.jion.forEach(function (user) {
                        var conn = allSockets[user];
                        sendTo(conn, {
                            event: 'call',
            ...
            case 'enterTheRoom':
                ...
                var o_inside = room[data.roomId].inside; //房间内的成员
                var o_outside = room[data.roomId].outside; //房间外的成员
                var outIndex = o_outside.findIndex(function (user) {
                ...
                //通知所有房间内成员新成员加入
                o_inside.forEach(function (user) {
                    if (user === data.name) {
                        return true;
                    }
                    var conn = allSockets[user];
            ...
            case 'leaveTheRoom':
                if (!!room[data.roomId]) {
                    var l_inside = room[data.roomId].inside;
                    var l_outside = room[data.roomId].outside;
                    var inIndex = l_inside.findIndex(function (user) {
                        return user == data.name;
                    });
                    if (inIndex >= 0) {
                    ...
                    l_inside.forEach(function (user) {
                        var conn = allSockets[user];
                        sendTo(conn, {
                            event: 'leaveTheRoom',
                            name: data.name
            ...
            case 'leave':
                ...
                if (!!data.user) {
                    var _conn2 = allSockets[data.user];
                    var _conn3 = allSockets[data.remoteUser];
                    ...
                    for (var _user2 in allUsers) {
                        var _conn4 = allSockets[_user2];
                        allUsers[_user2] = 1;
                        if (_conn4 != null) {
                            sendTo(_conn4, {
                                event: 'leave'
            ...
            case 'speechApply':
                ...
                break;
            case 'releaseSpeech':
                for (var _user3 in allUsers) {
                    var _conn5 = allSockets[_user3];
                    allUsers[_user3] = 1;
                    if (_conn5 != null) {
                        sendTo(_conn5, {
                            event: 'leave'
                ...
                // broadcast(server, allUsers);
                setTimeout(function () {
                    for (var _user4 in allUsers) {
                        if (applyUsers[_user4] === true) {
                            // console.log('Get Speech from ', user);
                            var _conn6 = allSockets[_user4];
                            sendTo(_conn6, {
                                event: 'getSpeech'
                            });
                            applyUsers[_user4] = false;
                            break;
                        }
                    }
                }, 1000);
...
function sendTo(conn, message) {
    if (!!conn) {
        conn.sendText((0, _stringify2.default)(message));
    } else {
        console.log('获取用户失败');
    }
}
function broadcast(server, info) {
    server.connections.forEach(function (conn) {
        conn.sendText((0, _stringify2.default)(info));
    });
}
// 处理录像流
var file_name = null;
...
function streamHandle(data) {
    if (data.event == 'StartRecord') {
        ...
        var camera_cnt = '' + yy + mm + dd + h + m + s + '_' + ss;
        ...
        setTimeout(function () {
            ...
            axios.post('http://192.168.20.31:18080/api/video/record/uploadFilePath', (_axios$post = { userId: data.id, associationId: 2 }, (0, _defineProperty3.default)(_axios$post, 'associationId', 0), (0, _defineProperty3.default)(_axios$post, 'startTime', startTime), (0, _defineProperty3.default)(_axios$post, 'endTime', data.time), (0, _defineProperty3.default)(_axios$post, 'filePath', 'videoRecord/'), (0, _defineProperty3.default)(_axios$post, 'fileName', file_name + '.webm'), _axios$post)).then(function (res) {
                if (res.code == 0) {
