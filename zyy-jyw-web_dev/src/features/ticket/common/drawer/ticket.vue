...
export default {
  ...
  methods: {
    ...
    onSelectEngineer(data) {
      TicketApi.putFind({ id: this.formData.info.id, userId: data.engineerId, }).then(res => {
        this.$emit('done');
      });
    },
    onCallInvalid() {
      TicketApi.putInvalid({ data: this.formData.info.id, }).then(res => {
        this.$emit('done');
      });
    },
    onCallResolved() {
      TicketApi.putPhoneEnd({ data: this.formData.info.id, }).then(res => {
        this.$emit('done');
      });
    },
    changeDeptId(deptId) {
      this.$nextTick(() => {
        this.$refs.relatedInfo.fetchInitData(deptId);
      });
    },
