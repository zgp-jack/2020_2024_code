        ...
        <div class="flex">
          ...
          <el-form-item label="工单来源">
            <dic-select
              ...
              :disabled="operatorType === 'EDIT' || createPattern === 'byPop'"
...
import { getOrderExpectedTimeConfigType, } from '@/features/system/otherConfig/api';
...
export default {
  ...
  data() {
    return {
      ...
      formData: {
        ...
        createTime: null,
        // 来源
        source: this.createPattern === 'byPop' ? 1 : 3,
        // 实际报修地址
        actualRepairPlace: '',
        // 故障分类
        faultType: null,
      ...
      pickerOptions: {
        disabledDate(time) {
          const now = new Date();
          const defaultTime = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 0, 0, 0);
          return time.getTime() < defaultTime.getTime();
        },
      },
    };
  },
  ...
  async created() {
    await this.getExpectedTimeType();
  ...
  methods: {
    ...
    /* 获取期望完成时间类型 */
    async getExpectedTimeType() {
      const result = await getOrderExpectedTimeConfigType();
      if (result != null) {
        this.formData.completeType = result;
      }
    },
    ...
    // 重置 清空
    reset() {
      const completeType = this.formData.completeType;
      this.formData = {
        // 建单人
