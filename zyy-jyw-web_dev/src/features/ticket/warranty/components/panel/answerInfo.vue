<template>
  ...
  >
    ...
    >
      ...
      >
        ...
        >
          <el-input
            ...
            :class="[createPattern !== 'warranty' && 'not-warranty']"
            :disabled="phoneDisabled"
            placeholder="请输入报修电话"
            @input="onPhoneChange"
          />
          <el-button
            v-if="createPattern !== 'warranty'"
            :disabled="!(formData.phone?.length === 4 || formData.phone?.length >= 7 && formData.phone?.length <= 8 || formData.phone?.length === 11)"
            class="add-btn"
            size="mini"
            type="primary"
            @click="changePhone"
      ...
      <el-form-item
        :rules="[{required: repair_place,pattern: /^(?:[0-9]{1,3}\.){3}[0-9]{1,3}$/,message: '请输入正确的IP地址', trigger: 'blur',}]"
        label="报修IP"
        prop="ipAddress"
      >
        <el-input v-model="formData.ipAddress" placeholder="请输入报修IP" />
      </el-form-item>
      <el-form-item
...
import PhoneDialog from '@/features/system/phone/components/phoneDialog.vue';
  ...
  methods: {
    changePhone() {
      this.$dialog({
        ...
        component: () => <PhoneDialog
          confirmFn={ApproveApi.submitPhone}
          // data={row}
          onDone={this.cancel}
          title={'报修电话信息维护'}
        />,
    ...
    async onDepartmentChange(data) {
      ...
      if (this.createPattern !== 'warranty' && data) {
        ...
        this.formData.ipAddress = this.restaurants[0]?.ipAddress;
      } else {
        this.restaurants = [];
      }
    },
