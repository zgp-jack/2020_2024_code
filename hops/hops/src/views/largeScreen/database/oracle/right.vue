<template>
	<div class="oracle-right">
				...
				<template v-slot:title>
					<div class="line-title">
						...
						<div class="line-title-legend" style="float: right">
							<div class="line-title-legend-item" style="margin-right: 16px">
								<div style="background-color: #006666"></div>
								上行
							</div>
							<div class="line-title-legend-item">
								<div style="background-color: #ffbc00"></div>
								下行
							</div>
						</div>
					</div>
		...
		<div class="large-card flex-1">
			<span slot="label"><i class="el-icon-date"></i> PGA内存</span>
			<LineMultipleChart :left="54" v-if="seriesData1.length" domId="lineMemPGAMultiple" :key="1" :seriesData="seriesData1" :unit="seriesData1Unit" height="100%">
			</LineMultipleChart>
		</div>
		<div class="large-card flex-1">
			<span slot="label"><i class="el-icon-date"></i> 其他内存</span>
			<LineMultipleChart :left="54" v-if="seriesData2.length" domId="lineMemOtherMultiple" :key="2" :seriesData="seriesData2" :unit="seriesData2Unit" height="100%">
			</LineMultipleChart>
		</div>
...
export default {
	...
	mounted() {
		...
		this.getChartData(6, 'seriesData1')
		this.getChartData(7, 'seriesData2')
