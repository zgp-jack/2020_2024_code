	...
	<div class="table-panel">
		<div class="main-box">
			<el-form :inline="true" :model="formInline" class="demo-form-inline">
				<el-form-item label="">
					<el-input v-model="formInline.computerName" placeholder="主机名称"></el-input>
				</el-form-item>
				<el-form-item label="">
					<el-input v-model="formInline.systemName" placeholder="版本"></el-input>
				</el-form-item>
				<el-form-item>
...
export default {
	...
	methods: {
		...
		getList() {
			let params = {
				name: this.formInline.computerName,
				systemName: this.formInline.systemName,
		...
		handleDown() {
			ApiHost.downExportTemp({}).then(res => {
				if(res.status === 1) {
		...
		handleExport() {
			...
			ApiHost.exportData(params).then(res => {
				if(res.status === 1) {
					window.open(`/file/${res.data}`);
				}
			})
