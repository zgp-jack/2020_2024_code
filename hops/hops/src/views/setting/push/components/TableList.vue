...
export default {
    ...
    methods: {
        ...
        getList() {
            ...
            ApiPush.page(params).then(res => {
                console.log(res);
                if(res.status === 1) {
                    const data = res.data;
                    this.total = data.total;
                    this.tableData = data.records.map(item => {
                        item.hostNumber = (item.hostIdList || []).length;
                        item.networkNumber = (item.networkIdList || []).length;
                        item.dbNumber = (item.dbIdList || []).length;
                        item.middlewareNumber = (item.middlewareIdList || []).length;
                        return item;
                    });
                }
            })
        ...
        handleDelete(row) {
            this.$confirm('确认删除该数据?', '提示', {
                ...
                ApiPush.del({userId: row.userId}).then(res => {
                    if(res.status) {
                        this.$message({
                            message: '删除成功',
                            type: 'success'
