 ...
 * @LastEditTime: 2022-03-18 13:58:07
 * @Description:
-->
<template>
          ...
          <el-row>
            <el-col :span="12">
              <el-form-item label="知识分类" prop="type">
                <dic-cascader v-model="formData.type" class="w-100" dic-key="zsk-konwlegeClassification" />
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="故障原因" prop="causes">
                <dic-select v-model="formData.causes" class="w-100" dic-key="zskFaultReason" />
              </el-form-item>
            </el-col>
          </el-row>
...
import DicSelect from '@/views/components/dictionary/dicSelect.vue';
import KnowledgeApi from '@/apis/knowledgeBase/retrieval/index';
import FormDialog, { OPERATOR_TYPES, } from '@/minxins/common/formDialog';
import DicCascader from '@/views/components/dictionary/dicCascader.vue';
...
export default {
  ...
  async created() {
    if (this.operatorType !== OPERATOR_TYPES.CREATE) {
      const data = await KnowledgeApi.detail(this.id);
  ...
  methods: {
    async onSubmit() {
      try {
        ...
        if (this.operatorType === OPERATOR_TYPES.CREATE) {
          await KnowledgeApi.create(this.formData);
        } else {
          await KnowledgeApi.update(this.formData);
        }
