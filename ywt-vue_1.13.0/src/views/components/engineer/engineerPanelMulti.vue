 ...
 * @CreateTime: 2021-08-28 11:58:00
 * @LastEditors: Pure <305737824@qq.com>
 * @LastEditTime: 2021-08-28 16:59:23
 * @Description:工程师选择面板(多选)
-->
<template>
  <div class="drawer">
    <div class="header">
      ...
      <div class="engineer">
        ...
        >
          <span slot-scope="{data}">
            ...
            <span class="pure-team-label">{{ data.name }}</span>
...
export default {
  ...
  async created() {
    try {
      ...
      // 如传入默认值,进行回显处理,为数组
      if (this.defaultSelectedData.length > 0) {
        this.dealWithDefaultSelectedData(this.defaultSelectedData);
      }
    } catch (error) {
      this.$message({
        type: 'error',
  ...
  methods: {
    ...
    // 处理传入的默认选项
    dealWithDefaultSelectedData(data) {
    ...
    confirm() {
      ...
      if (data.length > 0) {
        const engineerList = data.filter(item => !item.isTeam).map(item => {
          return {
            id: item.id,
            name: item.name,
          };
        });
        this.$emit('done', engineerList);
      } else {
        this.$message({
          type: 'error',
          message: '请选择工程师',
