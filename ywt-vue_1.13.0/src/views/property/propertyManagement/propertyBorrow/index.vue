 ...
 * @LastEditTime: 2021-11-17 11:02:32
 * @Description:资产转移
-->
<template>
  <el-container>
      ...
      <pure-table-area>
        <template slot="header">
          <div class="flex base-margin-left base-margin-top">
            ...
            />
            <pure-button label="借用申请" @click="handleCreate" />
          </div>
        </template>
        <template slot="body">
...
import Borrow from '@/views/property/propertyManagement/propertyBorrow/dialog/borrow.vue';
...
export default {
  ...
  data() {
    return {
      listApi: BorrowApi.list,
      tableHeader: [],
    };
  },

  async created() {
    this.tableHeader = this.$store.state['property/propertyManagement'].borrowTableColumn;
    this.fetchData();
  },
  methods: {
    handleView(row) {
      this.$drawer({
        width: '1688px',
        component: () => <Borrow onDone={this.fetchData} id={row.id} operatorType='VIEW'/>,
      });
    },
    handleCreate() {
      this.$drawer({
        width: '1688px',
        component: () => <Borrow onDone={this.fetchData} operatorType='CREATE'/>,
      });
    },
    handleUpdate(row) {
      this.$drawer({
        width: '1688px',
        component: () => <Borrow onDone={this.fetchData} id={row.id} operatorType='UPDATE'/>,
      });
    },
    convertState(state) {
