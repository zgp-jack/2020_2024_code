 ...
 * @LastEditTime: 2021-11-17 10:58:38
 * @Description:资产转移
-->
      ...
      <pure-table-area>
        <template slot="header">
          <div class="flex base-margin-left base-margin-top">
            ...
            />
            <pure-button label="转移申请" @click="handleCreate" />
          </div>
        </template>
        <template slot="body">
...
import Transfer from '@/views/property/propertyManagement/propertyTransfer/dialog/transfer.vue';
...
export default {
  ...
  data() {
    return {
      tableHeader: [],
      listApi: TransferApi.list,
    };
  },

  async created() {
    this.tableHeader = this.$store.state['property/propertyManagement'].transferTableColumn;
    this.fetchData();
  },
  methods: {
    handleView(row) {
      this.$drawer({
        width: '1688px',
        component: () => <Transfer onDone={this.fetchData} id={row.id} operatorType='VIEW'/>,
      });
    },
    handleCreate() {
      this.$drawer({
        width: '1688px',
        component: () => <Transfer onDone={this.fetchData} operatorType='CREATE'/>,
      });
    },
    handleUpdate(row) {
      this.$drawer({
        width: '1688px',
        component: () => <Transfer onDone={this.fetchData} id={row.id} operatorType='UPDATE'/>,
      });
    },
    convertState(state) {
