 ...
 * @CreateTime: 2021-09-18 11:52:54
 * @LastEditors: Pure <305737824@qq.com>
 * @LastEditTime: 2021-11-17 11:20:14
 * @Description:耗材出库管理
-->
      ...
      <pure-table-area>
        <template slot="header">
          <div class="flex base-margin-left base-margin-top">
            ...
            />
            <pure-button label="出库申请" @click="handleApplyWithdraw" />
          </div>
        </template>
        <template slot="body">
...
import ApplyForWithdraw from '@/views/property/consumable/dialog/applyForWithdraw.vue';
...
export default {
  ...
  methods: {
    async handleUpdate(row) {
      this.$drawer({
        width: '1688px',
        component: () => <ApplyForWithdraw onDone={this.fetchData} operatorType='UPDATE' id={row.id}/>,
      });
    },
    handleView(row) {
      this.$drawer({
        width: '1688px',
        component: () => <ApplyForWithdraw onDone={this.fetchData} operatorType='VIEW' id={row.id}/>,
      });
    },
    convertSupplier(id) {
    ...
    handleApplyWithdraw() {
      this.$drawer({
        width: '1688px',
        component: () => <ApplyForWithdraw onDone={this.fetchData} operatorType='CREATE'/>,
      });
    },
  },
};
