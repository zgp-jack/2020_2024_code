...
export default {
  ...
  methods: {
    ...
    setExpand() {
      this.SET_PANCEL_EXPAND({ name: 'technicalAnalysis', expand: true, });
    },
    async  save() {
      try {
        this.saveButtonLoading = true;
        await this.$refs['demandAssess'].onSave();
        this.$emit('done');
      } catch (error) {
    ...
    async  submit() {
      try {
        ...
        await this.$refs['demandAssess'].onSubmit();
        this.$emit('done');
      } catch (error) {
        this.$message.error(error);
      } finally {
