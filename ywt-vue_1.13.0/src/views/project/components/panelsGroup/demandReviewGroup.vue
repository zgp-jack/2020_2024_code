    ...
    <el-main ref="main" class="layout-panel-el-main-wrapper">
      <el-row class="layout-panel-el-main-wrapper-row">
        <el-col :span="10" class="layout-panel-el-main-wrapper-col P-R-16">
          <DemandReview ref="demandReview" :where="'2'" />
        </el-col>
        <el-col :span="10" class="layout-panel-el-main-wrapper-col P-R-16">
          <el-row>
...
export default {
  ...
  async created() {
    this.SET_PANCEL_EXPAND({ name: 'demandAssess', expand: true, });
  },
  mounted() {
    this.$nextTick(() => {
      this.computedHeight();
  ...
  methods: {
    ...
    async  onSave() {
      try {
        ...
        await this.$refs['demandReview'].onSave();
        // this.$emit('done');
      } catch (error) {
    ...
    async  submit() {
      try {
        ...
        await this.$refs['demandReview'].onSubmit();
        this.$emit('done');
      } catch (error) {
        this.$message.error(error);
      } finally {
