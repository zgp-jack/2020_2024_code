    ...
    <el-main ref="main" class="layout-panel-el-main-wrapper">
      <el-row class="layout-panel-el-main-wrapper-row">
        <el-col :span="10" class="layout-panel-el-main-wrapper-col P-R-16">
          <DemandAnalysis ref="demandAnalysis" :submit-fn="submit" />
        </el-col>
        <el-col :span="10" class="layout-panel-el-main-wrapper-col P-R-16">
          <el-row>
...
export default {
  ...
  methods: {
    ...
    setExpand() {
      if (this.shows.preliminaryLevel) {
        this.SET_PANCEL_EXPAND({ name: 'preliminaryLevel', expand: true, });
      } else {
    ...
    async save() {
      try {
        ...
        await this.$refs['demandAnalysis'].onSave();
        this.$emit('done');
      } catch (error) {
    ...
    async submit() {
      try {
        ...
        await this.$refs['demandAnalysis'].onSubmit();
        this.$emit('done');
      } catch (error) {
        this.$message.error(error);
      } finally {
