 ...
 * @CreateTime: 2021-07-23 16:27:37
 * @LastEditors: Pure <305737824@qq.com>
 * @LastEditTime: 2022-01-20 10:20:07
 * @Description:科室报修情况(工单数量)
-->
<template>
  <el-card>
    <el-row>
      <el-col :span="12">
        <span class="label">科室报修情况(工单数量)</span>
    ...
    <el-row>
      <el-col :span="24">
        <div id="department-echart" class="echart" :style="{float:'left',width: '100%', height: '290px'}" />
      </el-col>
    </el-row>
  </el-card>
</template>
...
export default {
  ...
  async mounted() {
    const chartDom = document.getElementById('department-echart');
  ...
  methods: {
    ...
    getFirst25() {
      ...
      this.unsortData.descDept.map((item) => {
        data.deptNames.push(item.deptIdToName);
        data.orderAmount.push(item.orderAmount);
      });
    ...
    getLast25() {
      ...
      this.unsortData.ascDept.map((item) => {
        data.deptNames.push(item.deptIdToName);
        data.orderAmount.push(item.orderAmount);
      });
      return data;
    ...
    async fetchData() {
      try {
        this.unsortData = await StatisticsApi.deptReport({ deptId: this.deptId, startTime: this.startTime, endTime: this.endTime, });
    ...
    genOption(type) {
      const show = this.data?.deptNames?.length === 0 && this.data?.orderAmount?.length === 0;
      const option = {
        title: {
          show: show,
          text: '暂无数据',
