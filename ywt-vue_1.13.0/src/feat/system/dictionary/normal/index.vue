 ...
 * @CreateTime: 2021-12-28 17:11:59
 * @LastEditors: Pure <305737824@qq.com>
 * @LastEditTime: 2022-10-08 16:54:35
 * @Description:普通字典
-->
<template>
...
export default {
  ...
  methods: {
    ...
    async  fetchCategory() {
      const { list, } = await DicNormalApi.list();
    ...
    async lazyLoad(tree, treeNode, resolve) {
      const { list, } = await DicNormalApi.itemList({
    ...
    async onChangeItemState(row) {
      ...
      await DicNormalApi.updateItem(data);
    ...
    async onDeleteItem(row) {
      try {
        await this.$confirm(`确认删除'${row.name}'项吗？`, '提示', { confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning', });
        await DicNormalApi.deleteItem(row.id);
