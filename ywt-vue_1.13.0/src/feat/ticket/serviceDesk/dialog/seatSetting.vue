...
import DutyManagerApi from '@/feat/synthesize/workforceManagement/api';
...
export default {
  ...
  methods: {
    ...
    async fetchCurrentDutyWatchman() {
      try {
        const data = await DutyManagerApi.currentDutyWatchman();
    ...
    setCallDivert(phone, engineersNeedToNotify) {
      if (this.formData.divertStatus === 1) {
        this.$store.state.ticketServiceDesk.ws.send({ code: 18, data: phone, });
      } else {
        this.$store.state.ticketServiceDesk.ws.send({ code: 18, data: '', });
      }
      if (this.formData.wechatTicketNotifyToggle === 1) {
        this.$store.state.ticketServiceDesk.ws.send({ code: 19, data: engineersNeedToNotify, });
      } else {
        this.$store.state.ticketServiceDesk.ws.send({ code: 19, data: '', });
      }
    },
