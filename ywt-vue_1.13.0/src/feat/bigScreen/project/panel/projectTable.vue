<template>
  <Wrapper title="项目总体视图">
    <Table v-loading="loading" element-loading-background="#01070da6" :data="tableData" @rowClick="rowClick">
      ...
      <el-table-column prop="priority" label="项目优先级" width="120px">
        <template #default="{row}">
          <span :style=" row.priority | textColor">
            {{ row.priority | dicValueToKey('itemPriority') }}
          </span>
        </template>
      </el-table-column>
      <el-table-column prop="totalProgress" label="当前进度">
        <template #default="{row}">
          <Bar :value="row.totalProgress || 0" show-per />
        </template>
      </el-table-column>
...
import PanelsGroup from '../component/drawerPanel/index.vue';
