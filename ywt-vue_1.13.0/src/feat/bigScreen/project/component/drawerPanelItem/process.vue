    ...
    <el-main>
      <el-timeline class="process-timeline">
        ...
        >
          ...
          <div>
            <Button v-if="v.hasVote" :type="'info'" @click="clickVoteResult(v.id)">
              投票结果
            </Button>
          </div>
          <Button
            v-if="v.processState"
            class="m-t-10"
            :type="isInfo.includes(v.processState) ?'info':'default'"
            @click="clickProcessState(v)"
          >
            {{ v.processState | projectProcessState("处理任务") }}
          </Button>
          <Button v-else-if="v.process" class="m-t-10">
            {{ v.process | needStatus }}
          </Button>
          ...
          <template v-if="v.processState === 'demand_dist'">
            ...
            <h6 class="name">调研参与人:{{ JSON.parse(v.value).userNames }}</h6>
          </template>
        </el-timeline-item>
      </el-timeline>
    </el-main>
...
export default {
  ...
  data() {
    return {
      ...
      voteData: [],
      visibleVote: false,
      isInfo: ['demand_submit', 'investigate_submit', 'item_submit', 'ORDER_MAKE', 'ORDER_FAULT'],
    };
  },
  ...
  methods: {
    ...
    clickProcessState(data) {
      if (this.isInfo.includes(data.processState)) {
