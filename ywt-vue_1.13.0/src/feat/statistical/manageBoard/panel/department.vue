 ...
 * @CreateTime: 2022-09-05 21:49:34
 * @LastEditors: Pure <305737824@qq.com>
 * @LastEditTime: 2022-09-09 21:58:20
 * @Description:工作量统计-科内
-->
<template>
  <el-card style="height:100%;">
    <el-row>
      <el-col :span="12">
        ...
        <pure-button-group style="display:inline" :options="groupBtnOptions" default-active="按小组" @change="handleTabChange" />
      </el-col>
    ...
    <el-row class="base-margin-top" style="height:100%">
      <el-col :span="24" style="height:100%;position:absolute;padding-bottom:30px">
        ...
        >
          ...
          <el-table-column label="小组" prop="name" show-overflow-tooltip="" />
          <el-table-column label="服务台">
            <el-table-column label="接听" prop="serviceAnswer">
              <template slot-scope="{row}">
                <el-tag size="mini" type="danger">{{ row.serviceAnswer }}</el-tag>
              </template>
            </el-table-column>
            <el-table-column label="电话解决" prop="serviceSolve" width="100">
              <template slot-scope="{row}">
                <el-tag size="mini" type="danger">{{ row.serviceSolve }}</el-tag>
              </template>
            </el-table-column>
            <el-table-column label="派单" prop="serviceSend">
              <template slot-scope="{row}">
                <el-tag size="mini" type="danger">{{ row.serviceSend }}</el-tag>
              </template>
            </el-table-column>
            <el-table-column label="超时未派" prop="serviceOvertime" width="95">
              <template slot-scope="{row}">
                <el-tag size="mini" type="danger">{{ row.serviceOvertime }}</el-tag>
              </template>
            </el-table-column>
          </el-table-column>
          ...
          <el-table-column label="巡检">
            <el-table-column label="完成" prop="inspectionComplete">
              <template slot-scope="{row}">
  ...
  methods: {
    async onDownExcel() {
      try {
        ...
        const fileName = `工作量统计-${this.startTime}__${this.endTime}.xlsx`;
