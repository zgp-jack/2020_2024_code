 ...
 * @LastEditTime: 2022-10-08 11:49:13
 * @Description:统计分析-常见故障
-->
<template>
  <el-card>
    <el-row>
      <el-col :span="12">
        <span class="label">常见故障TOP5</span>
      </el-col>
...
export default {
  ...
  data() {
    return {
      ...
      progressOption: {
        ...
        // innerTitle: ['显示器不显示了 ', '自助机没出纸', '打印机没工作', '键盘坏了', '网络无链接'],
        innerTitle: [],
      },
      startTime: null,
      endTime: null,
  ...
  // 查询条件变更后操作
    ...
    async fetchData() {
      ...
      const data = await TicketApi.commonFaultTopFive({ startDate: this.startTime, endDate: this.endTime, });
      data.forEach(item => {
        ...
        this.progressOption.innerTitle.unshift(item.faultTypeToName);
      });
    },
    getImg() {
      const fileName = `常见故障`;
