 ...
 * @CreateTime: 2023-01-10 10:32:52
 * @LastEditors: Pure <305737824@qq.com>
 * @LastEditTime: 2023-01-10 10:40:07
 * @Description:服务时效性分析
-->
...
<template>
  <el-card style="height:598px">
    ...
    >
      ...
      <el-table-column label="优秀" prop="niceTotal">
        <template slot-scope="{row}">
          <el-tag size="mini" type="info">{{ row.niceTotal }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="达标" prop="standardTotal">
        <template slot-scope="{row}">
          <el-tag size="mini" type="info">{{ row.standardTotal }}</el-tag>
        </template>
      </el-table-column>

      <el-table-column label="不达标 " prop="noStandardTotal">
        <template slot-scope="{row}">
          <el-tag size="mini" type="danger">{{ row.noStandardTotal }}</el-tag>
        </template>
      </el-table-column>
...
export default {
  ...
  data() {
    return {
      tableData: [],
      list: [],
      listApi: TicketApi.serviceTimeliness,
      groupBtnOptions: [{
        count: null,
        name: '按人员',
      }, {
  ...
  methods: {
    async onDownExcel() {
      try {
        ...
        const res = await TicketApi.serviceTimelinessExportExcel(params);
        const fileName = `服务时效性统计分析-[${this.nameLabel}]}.xlsx`;
