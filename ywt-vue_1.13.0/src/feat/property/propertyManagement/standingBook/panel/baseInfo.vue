 ...
 * @LastEditTime: 2023-04-17 17:04:03
    ...
    >
        ...
        <el-row>
          ...
          <el-col :span="25">
            ...
            <el-form-item v-else label="资产编号" prop="assetsCode">
              <el-input v-model="formData.assetsCode" :disabled="operatorType!=='CREATE'" />
            </el-form-item>
          </el-col>
          <el-col :span="25">
        ...
        <el-row>
          ...
          <el-col :span="25">
            <el-form-item label="使用责任人">
              <engineer-select v-model="formData.personLiable" :multiple="false" />
            </el-form-item>
          </el-col>
        </el-row>
      </el-form-item>
        ...
        <el-row>
          ...
          <el-col :span="50">
            <el-form-item label="维保公司" prop="maintainCompany">
              ...
              <common-dept-cascader v-model="formData.maintainCompany" class="w-100" :source="manufacturerList" @change="onManufacturerChange" />
            </el-form-item>
          </el-col>
          <el-col :span="25">
            <el-form-item label="维保责任人" prop="maintainUser">
              <el-select v-model="formData.maintainUser" :disabled="!manufacturerUserList.length" class="w-100">
                <el-option v-for="(engineerItem,engineerIndex) in manufacturerUserList" :key="engineerIndex" :label="engineerItem.name" :value="engineerItem.id" />
              </el-select>
            </el-form-item>
          </el-col>
        </el-row>
      ...
      <el-form-item label-width="0px" class="form-item-wrapper">
        <el-row>
          ...
          <el-col :span="25">
            <el-form-item label="所属巡检单元">
              <inspection-unit-cascader v-model="formData.pertainUnit" />
            </el-form-item>
          </el-col>
          <el-col :span="25">
...
export default {
  components: {
    ...
    InspectionTemplateSelect: () => import('@/views/components/inspection/templateSelect.vue'),
    InspectionUnitCascader: () => import('@/views/components/inspection/unitCascader.vue'),
    TreeDicCascader,
  },
  props: {
    showExpand: {
  ...
  data() {
    return {
      ...
      // 厂家列表
      manufacturerList: this.$store.state.property.manufacturerList,
      // 厂家代表列表
  ...
  computed: {
    rules() {
      ...
      if (this.formData.isMetering) {
        data.meteringTimer = [{
          required: true, message: '请输入计量周期', trigger: 'blur',
        }];
      }
      if (this.formData.isPatrol) {
        data.patrolTemp = [{
          required: true, message: '请选择巡检模板', trigger: 'change',
        }];
      }
      if (this.formData.isAutoCode) {
        data.codePrefix = [{
          required: true, message: '请输入资产编号前缀', trigger: 'blur',
        }, {
          pattern: /^[A-Za-z0-9]{1,6}$/, message: '1-6位的数字和大小写英文', trigger: 'blur',
        }];
      } else {
        data.assetsCode = [{
  ...
  methods: {
    ...
    async check() {
      ...
      this.formData.sendTeam = sendTeam;
    ...
    async initManufacturerInfo() {
      if (this.formData.maintainCompany) {
        const { list, } = await await EngineerApi.list({ pageNum: 0, pageSize: 9999, deptId: this.formData.maintainCompany, });
        this.manufacturerUserList = list;
      }
    },

    // 维修厂家变更
    async onManufacturerChange(val, index) {
      const { list, } = await EngineerApi.list({ pageNum: 0, pageSize: 9999, deptId: val.id, });

      this.manufacturerUserList = list;
      if (list.length === 0) {
        return this.$message.warning('此公司无用户！');
      }
      this.formData.maintainUser = list[0]?.id;
    },
  },
};
</script>
